...
import type { GeneratePostRequest, GeneratePostResponse } from '@/types/social-platforms';
...
Generate engaging, platform-optimized content that drives real engagement.`;
    ...
    console.log(`🤖 Generating ${body.platform} post: ${body.postType} about "${body.topic}"`);
    ...
    console.log(`✅ Generated content - Tokens: ${totalTokens} (in: ${inputTokens}, out: ${outputTokens}), Cost: $${cost.toFixed(6)}`);
    ...
    if (contentLength > platformSpec.text.maxChars) {
      console.warn(`⚠️ Generated content exceeds limit: ${contentLength}/${platformSpec.text.maxChars}`);
    }

    if (hashtagCount > platformSpec.hashtags.max) {
      console.warn(`⚠️ Generated hashtags exceed limit: ${hashtagCount}/${platformSpec.hashtags.max}`);
      // Truncate to limit
      generatedPost.hashtags = generatedPost.hashtags.slice(0, platformSpec.hashtags.max);
    }
    ...
    const responseData = {
      ...
      post: {
        ...
        metadata: {
          estimatedEngagement: generatedPost.metadata?.estimatedEngagement || 'medium',
    ...
    console.log(`✅ Post generation completed in ${responseData.generationTimeMs}ms`);
