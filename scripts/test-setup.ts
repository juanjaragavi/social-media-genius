...
import { getSupportedPlatforms, getPlatformSpec } from '../lib/social-platform-specs';
...
async function runTests() {
  ...
  platforms.forEach(platform => {
    ...
    if (spec) {
      ...
      console.log(`  - Hashtag limit: ${spec.hashtags.max} (recommended: ${spec.hashtags.recommended})`);
  ...
  const instagramValidation = validatePost('instagram', testContent, testHashtags);
  console.log('Instagram post validation:');
  console.log(`  - Valid: ${instagramValidation.valid ? '✓' : '✗'}`);
  console.log(`  - Char count: ${instagramValidation.stats.charCount}/${instagramValidation.stats.charLimit}`);
  console.log(`  - Hashtags: ${instagramValidation.stats.hashtagCount}/${instagramValidation.stats.hashtagLimit}`);
