...
export class VeoService {
  ...
  private getPlatformVideoSettings(platform: string): {
    ...
    const settingsMap: Record<string, { aspectRatio: string; duration: number }> = {
    ...
    return settingsMap[platform.toLowerCase()] || { aspectRatio: '16:9', duration: 30 };
  }

  /**
   * Enhance video prompt based on style
   */
  private enhanceVideoPrompt(basePrompt: string, style: string, platform: string): string {
    const styleEnhancements: Record<string, string> = {
      cinematic: 'cinematic camera movements, professional lighting, smooth transitions, high production value',
      'fast-paced': 'quick cuts, dynamic movement, energetic pacing, rapid scene changes, trending style',
      smooth: 'smooth camera movements, steady shots, gentle transitions, professional flow',
      dynamic: 'dynamic angles, varied perspectives, engaging movement, attention-grabbing visuals',
    };

    const platformHints: Record<string, string> = {
      tiktok: 'trending TikTok style, mobile-first, hook in first 3 seconds, vertical format',
      instagram: 'Instagram Reels aesthetic, polished, visually appealing, mobile-optimized',
  ...
  async generateVideo(options: VideoGenerationOptions): Promise<VideoGenerationResult> {
    ...
    try {
      ...
      const durationSeconds = options.durationSeconds || platformSettings.duration;
      ...
      console.log('⚠️  Veo 3.1: Video generation is in preview. Using placeholder response.');
      ...
      return {
        success: false,
        error: 'Veo 3.1 video generation is not yet available in this SDK version. Please check Google Cloud documentation for the latest API.',
        metadata: {
          sizeBytes: 0,
          sizeMB: 0,
          durationSeconds,
