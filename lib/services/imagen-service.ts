...
export class ImagenService {
   ...
   */
  private static readonly VALID_ASPECT_RATIOS = ['1:1', '3:4', '4:3', '9:16', '16:9'] as const;
  ...
  private getPlatformAspectRatio(platform: string): string {
    ...
    const aspectRatioMap: Record<string, typeof ImagenService.VALID_ASPECT_RATIOS[number]> = {
    ...
    return aspectRatioMap[platform.toLowerCase()] || ImagenService.DEFAULT_ASPECT_RATIO;
  }

  /**
   * Validate and normalize aspect ratio to Imagen-supported values
   */
  private validateAspectRatio(aspectRatio: string): typeof ImagenService.VALID_ASPECT_RATIOS[number] {
    const normalized = aspectRatio.trim();
    
    // Direct match
    if (ImagenService.VALID_ASPECT_RATIOS.includes(normalized as typeof ImagenService.VALID_ASPECT_RATIOS[number])) {
      return normalized as typeof ImagenService.VALID_ASPECT_RATIOS[number];
    }
    
    // Map unsupported ratios to closest supported ones
    const fallbackMap: Record<string, typeof ImagenService.VALID_ASPECT_RATIOS[number]> = {
    ...
    if (fallback) {
      console.log(`⚠️ Imagen: Aspect ratio ${normalized} not supported, using ${fallback}`);
      return fallback;
    }
    
    console.log(`⚠️ Imagen: Unknown aspect ratio ${normalized}, using ${ImagenService.DEFAULT_ASPECT_RATIO}`);
  ...
  private enhancePrompt(basePrompt: string, style: string): string {
    const styleEnhancements: Record<string, string> = {
      ...
      minimalist: 'minimalist, clean, simple, elegant, professional, uncluttered',
  ...
  async generateImage(options: ImageGenerationOptions): Promise<ImageGenerationResult> {
    ...
    try {
      const rawAspectRatio = options.aspectRatio || this.getPlatformAspectRatio(options.platform);
      ...
      console.log(`✅ Imagen 4.0: Generated successfully in ${generationTimeMs}ms`);
  ...
  async optimizeForPlatform(
    ...
    try {
      ...
      console.log(`🔧 Optimizing image: ${sizeMB.toFixed(2)} MB → ${targetSizeMB} MB target`);
